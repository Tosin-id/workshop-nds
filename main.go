...
func main() {
	...
	ucl := client.NewUmurClientStruct(logger)
	uct := controller.NewUmurController(logger, ucl)
	usv := service.NewUmurService(logger, uct)
	proto.RegisterUmurServer(grpcServer, usv)
